...
#pragma mark - Voice
  class Voice : public SynthVoice
    ...
    public:
        ...
        void Trigger(double level, bool isRetrigger) override
        {
            ...
            if(isRetrigger){
                ...
                filterEnv.Retrigger(level);
            }
            else{
                mAMPEnv.Start(level);
                filterEnv.Start(level);
            }
        }
        ...
        void ProcessSamplesAccumulating(T** inputs, T** outputs, int nInputs, int nOutputs, int startIdx, int nFrames) override
        {
            ...
            for(auto i = startIdx; i < startIdx + nFrames; i++)
            {
                ...
                mFilter.setCutoffMod(filterEnv.Process() * filterEnvelopeAmount); // filterEnv.Process() * filterEnv.Process() -> power envelope excursion
                outputs[0][i] += mFilter.process(mOSC.nextSample() * mAMPEnv.Process(inputs[kModSustainSmoother][i])) * mGain;
        ...
        void setMode(int indx) {
            mOSC.setMode(Oscillator::OscillatorMode(indx));
        }
    ...
    public:
        ...
        // FILTER
        Filter mFilter;
        ADSREnvelope<sample> filterEnv;// ("env", nullptr, false);
...
public:
#pragma mark -
  a2_MidiNoteFilterModDSP(int nVoices)
    {
        ...
        switch (paramIdx) {
            ...
            case kParamRelease:
            {
                ...
                mSynth.ForEachVoice([stage, value](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).mAMPEnv.SetStageTime(stage, value);
                });
                break;
            }
                //waveform
            case kParamMode:
            {
                //            int v = (OscillatorMode)GetParam(kParamMode)->Int();
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).setMode(value);
                });
            }
                break;
                // filter
            case mFilterCutoff:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).mFilter.setCutoff(value);
                });
                break;
            case mFilterResonance:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).mFilter.setResonance(value);
                });
                break;
            case mFilterMode:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).mFilter.setFilterMode(static_cast<Filter::FilterMode>(value));
                });
                break;
            case mAttenuationMode:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).mFilter.setAttenuation(static_cast<Filter::FilterAttenuation>(value));
                });
                break;
            case mFilterAttack:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).filterEnv.SetStageTime(ADSREnvelope<sample>::kAttack, value);
                });
                break;
            case mFilterDecay:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).filterEnv.SetStageTime(ADSREnvelope<sample>::kDecay, value);
                });
                break;
            case mFilterSustain:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).filterEnv.SetStageTime(ADSREnvelope<sample>::kSustain, value);
                });
                break;
            case mFilterRelease:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).filterEnv.SetStageTime(ADSREnvelope<sample>::kRelease, value);
                });
                break;
            case mFilterEnvelopeAmount:
                mSynth.ForEachVoice([&](SynthVoice& voice) {
                    dynamic_cast<a2_MidiNoteFilterModDSP::Voice&>(voice).filterEnvelopeAmount = value;
                });
                break;
            default:
                break;
