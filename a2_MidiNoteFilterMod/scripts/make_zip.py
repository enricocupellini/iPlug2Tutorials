...
def  main():
  ...
  installer = "\installer\a2_MidiNoteFilterMod Installer.exe"
   
  if demo:
    installer = "\installer\a2_MidiNoteFilterMod Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\a2_MidiNoteFilterMod manual.pdf" 
  ]
  ...
  ZIPNAME = "a2_MidiNoteFilterMod-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "a2_MidiNoteFilterMod-v" + FULLVERSIONSTR + "-win-demo.zip"
