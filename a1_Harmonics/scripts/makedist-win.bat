...
if %1 == 1 (echo Making a1_Harmonics Windows DEMO VERSION distribution ...) else (echo Making a1_Harmonics Windows FULL VERSION distribution ...)
...
REM - msbuild a1_Harmonics-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild a1_Harmonics.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild a1_Harmonics.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\a1_Harmonics.aaxplugin\Contents\Win32\a1_Harmonics.aaxplugin --out .\build-win\aax\bin\a1_Harmonics.aaxplugin\Contents\Win32\a1_Harmonics.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\a1_Harmonics.aaxplugin\Contents\x64\a1_Harmonics.aaxplugin --out .\build-win\aax\bin\a1_Harmonics.aaxplugin\Contents\x64\a1_Harmonics.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\a1_Harmonics.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\a1_Harmonics.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "a1_Harmonics Installer" ".\installer\a1_Harmonics Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\a1_Harmonics Installer.exe" ".\installer\a1_Harmonics Demo Installer.exe"
REM -del ".\installer\a1_Harmonics Installer.exe"
REM -)
