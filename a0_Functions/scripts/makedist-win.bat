...
if %DEMO_ARG% == "demo" (
  echo Making a0_Functions Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making a0_Functions Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild a0_Functions-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild a0_Functions.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild a0_Functions.sln /t:a0_Functions-app;a0_Functions-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\a0_Functions.aaxplugin\Contents\Win32\a0_Functions.aaxplugin --out .\build-win\aax\bin\a0_Functions.aaxplugin\Contents\Win32\a0_Functions.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\a0_Functions.aaxplugin\Contents\x64\a0_Functions.aaxplugin --out .\build-win\aax\bin\a0_Functions.aaxplugin\Contents\x64\a0_Functions.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\a0_Functions.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\a0_Functions.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "a0_Functions Installer" ".\installer\a0_Functions Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\a0_Functions Installer.exe" ".\installer\a0_Functions Demo Installer.exe"
  REM -del ".\installer\a0_Functions Installer.exe"
  REM -)
